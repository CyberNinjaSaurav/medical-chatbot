...
from medical_chatbot.messaging.kafka_producer import KafkaJSONProducer

PROJECT_ROOT = Path(__file__).resolve().parents[3]
PLATFORM_ROOT = PROJECT_ROOT / "platform"
LEGACY_WEB_ROOT = PLATFORM_ROOT / "legacy-web"
LEGACY_TEMPLATE_DIR = LEGACY_WEB_ROOT / "templates"
LEGACY_STATIC_DIR = LEGACY_WEB_ROOT / "static"
...
CATALOG_UPLOAD_DIR = LEGACY_STATIC_DIR / "catalogue_uploads"
...
app = Flask(
    __name__,
    template_folder=str(LEGACY_TEMPLATE_DIR),
    static_folder=str(LEGACY_STATIC_DIR),
)
app.secret_key = FLASK_SECRET_KEY
CORS(app)

PLATFORM_DIST_DIR = PLATFORM_ROOT / "dist"
PLATFORM_ASSETS_DIR = PLATFORM_DIST_DIR / "assets"
...
@app.route("/")
def index():
    if (PLATFORM_DIST_DIR / "index.html").is_file():
        return send_from_directory(str(PLATFORM_DIST_DIR), "index.html")
...
def platform_assets(filename: str):
    if PLATFORM_ASSETS_DIR.is_dir():
        return send_from_directory(str(PLATFORM_ASSETS_DIR), filename)
    return "Platform assets not found. Run `npm run build` inside `platform/`.", 404


@app.route("/<path:filename>")
def serve_platform_file(filename: str):
    file_path = PLATFORM_DIST_DIR / filename
    if file_path.is_file():
        return send_from_directory(str(PLATFORM_DIST_DIR), filename)

    if not filename.startswith("get") and (PLATFORM_DIST_DIR / "index.html").is_file():
        return send_from_directory(str(PLATFORM_DIST_DIR), "index.html")
